#!/usr/bin/env python3
"""
Training script for DDQN agent on 2-player Snake game
"""
...
from ddqn_agent import train_ddqn_agent, test_ddqn_agent

def main():
    parser = argparse.ArgumentParser(description='Train DDQN agent for 2-player Snake game')
    ...
    parser.add_argument('--model-path', type=str, default='models/ddqn_snake.pth', help='Path to save/load model (legacy)')
    ...
    parser.add_argument('--hidden-size', type=int, default=256, help='Hidden dimension size for the MLP')
    ...
    if args.test_only:
        print("Testing existing DDQN model...")
        test_scores, test_lengths = test_ddqn_agent(
